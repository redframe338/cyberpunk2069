...
$source = Join-Path $root 'assets\china.png'
...
foreach ($folder in $sizes.Keys) {
	...
	foreach ($suffix in $variants) {
		Save-Hoi4Tga $image $dimensions[0] $dimensions[1] (Join-Path $outputDirectory "CHI$suffix.tga")
	}
}

$image.Dispose()
$stream.Dispose()
Write-Host 'Installed 15 People''s Republic Of China flag files.'

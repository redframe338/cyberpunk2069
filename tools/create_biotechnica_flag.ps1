...
$sourcePath = "C:\Users\victo\Downloads\IMG_6603.jpeg"
...
$graphics.Clear([System.Drawing.Color]::FromArgb(14, 18, 17))

# Crop only excess background so the supplied logo remains proportional.
...
$graphics.DrawImage(
    $source,
    [System.Drawing.Rectangle]::new(0, 0, 820, 520),
...
foreach ($size in $sizes) {
    foreach ($suffix in @("", "_neutrality", "_democratic", "_fascism", "_communism")) {
        Save-Tga $canvas $size.width $size.height (Join-Path $size.directory "BIO$suffix.tga")
    }
}

$canvas.Dispose()
Write-Host "Installed 15 bottom-origin Biotechnica flag files."

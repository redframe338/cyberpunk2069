...
{	
	...
	[[		
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			// Dark dystopian continental pass.
			// Keep the terrain readable beneath political colours while pushing
			// every landmass toward charcoal, ink blue and muted petrol green.
			float vTerrainLuma = dot( vOut, float3( 0.2126f, 0.7152f, 0.0722f ) );
			float3 vDystopianTerrain = float3(
				vTerrainLuma * 0.19f,
				vTerrainLuma * 0.23f,
				vTerrainLuma * 0.28f
			);
			float vGradeStrength = 0.76f * ( 1.0f - saturate( vBloomAlpha ) * 0.82f );
			vOut = lerp( vOut, vDystopianTerrain, vGradeStrength );
			vOut *= 0.82f;

			// Extremely faint survey grid; arithmetic only, with no extra pass.
			float2 vGridCell = abs( frac( Input.uv2 * float2( 88.0f, 32.0f ) ) - 0.5f );
			float vGridLine = 1.0f - saturate( min( vGridCell.x, vGridCell.y ) * 180.0f );
			vOut += float3( 0.002f, 0.006f, 0.008f ) * vGridLine * ( 1.0f - saturate( vBloomAlpha ) );
